CREATE PROCEDURE [dbo].[spPopulateFromQuickenStaging]
AS
BEGIN
	...
	USING (
		SELECT 
			...
			qst.Reconciled, 
			qst.Cleared, 
			CASE WHEN ISNUMERIC(qst.QuickenCheckNumber) = 1 AND CAST(qst.QuickenCheckNumber AS int) BETWEEN 200 AND 6000 THEN qst.QuickenCheckNumber END AS CheckNumber, 
			qst.QuickenMemo, 
			qst.QuickenCheckNumber
		FROM QuickenStagingTransaction qst
		INNER JOIN Account a ON qst.AccountName = a.AccountName
	) src
	ON 1 = 0
	WHEN NOT MATCHED THEN
		INSERT (AccountID, TransactionSerialNumber, TransactionDate, FriendlyDescription, Amount, Balance, Reconciled, Cleared, CheckNumber, InQuicken, QuickenMemo, QuickenCheckNumber)
		VALUES (src.AccountID, src.TransactionSerialNumber, src.TransactionDate, src.FriendlyDescription, src.Amount, src.Balance, src.Reconciled, src.Cleared, src.CheckNumber, 1, src.QuickenMemo, src.QuickenCheckNumber)
	OUTPUT
	...
	USING (
		SELECT 
			...
			a.AccountID,
			qszr.ReferenceDate
		FROM QuickenStagingZeroRecord qszr
		INNER JOIN Account a ON qszr.AccountName = a.AccountName
	) src
	ON 1 = 0
	WHEN NOT MATCHED THEN
		INSERT (AccountID, ReferenceDate, InQuicken)
	...
	AND NOT EXISTS (SELECT 1 FROM QuickenExcludeCategories WHERE CategoryName = qsts.CategoryName)
	...
	FROM QuickenStagingTransactionSplit qsts
