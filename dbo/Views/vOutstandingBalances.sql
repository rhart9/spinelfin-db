...
AS
	SELECT COALESCE(c.ReconGroupDescription, c.Description, '[No Category]') AS Category, SUM(s.Amount) AS Amount, CASE WHEN (c.IsClosed = 1 OR c.IsClosedInLegacy = 1) THEN 'CLOSED' ELSE '' END AS Closed
	...
	GROUP BY COALESCE(c.ReconGroupDescription, c.Description, '[No Category]'), c.IsClosed, c.IsClosedInLegacy
