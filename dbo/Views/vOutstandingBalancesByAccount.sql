...
CREATE VIEW [dbo].[vOutstandingBalancesByAccount]  
AS
	SELECT a.AccountName, COALESCE(c.ReconGroupDescription, c.Description, '[No Category]') AS Category, SUM(s.Amount) AS Amount, CASE WHEN c.IsClosed = 1 THEN 'CLOSED' ELSE '' END AS Closed
	...
	GROUP BY a.AccountName, COALESCE(c.ReconGroupDescription, c.Description, '[No Category]'), c.IsClosed
